...
var $oConexao;
	...
	*/
	function recupera($nIdTipoTransacao,$nIdGrupoUsuario) {
		...
		if ($oReg) {
			$oPermissao = new Permissao($oReg->id_tipo_transacao,$oReg->id_grupo_usuario,$oReg->dt_permissao,$oReg->publicado,$oReg->ativo);
			return $oPermissao;
		}
		return false;
	}
	...
	*/
	function insere($oPermissao) {
		$oConexao = $this->getConexao();
		$sSql = "insert into seg_permissao (id_tipo_transacao,id_grupo_usuario,dt_permissao,publicado,ativo) 
				 values ('".$oPermissao->getIdTipoTransacao()."','".$oPermissao->getIdGrupoUsuario()."','".$oPermissao->getDtPermissao()."','".$oPermissao->getPublicado()."','".$oPermissao->getAtivo()."')";		
		$oConexao->execute($sSql);		
	...
	*/
	function altera($oPermissao) {
		...
		$sSql = "update seg_permissao 
				 set    dt_permissao = '".$oPermissao->getDtPermissao()."',
