...
class Login {
	...
	function logarUsuarioPainel($sLogin,$sSenha,$sBanco){
		...
		$vWhereUsuario = array("login = '".utf8_encode($sLogin)."'","publicado = 1","ativo = 1");
			...
			if(is_object($oUsuario)){
				if ($oUsuario->getSenha() == $sSenha){
					...
					if($oUsuario->getLogado() == 1){
						...
						$sObjetoAcesso = "VERIFICAR ERRO 1: Tentativa de Login falhou. USUARIO LOGADO NO SISTEMA. Login do usuário: ".utf8_decode($_POST['fLogin'])." Senha: ".$_POST['fSenha'];
						$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacaoAcesso("",$nIdTipoTransacaoAcesso,$oUsuario->getId(),utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
						...
						$nIdTipoTransacaoAcesso = $oFachadaSeguranca->recuperaTipoTransacaoPorDescricaoCategoria("Acesso","Login",$sBanco);
						$sObjetoAcesso = "Login efetuado com sucesso. Login do usuário: ".utf8_decode($_POST['fLogin']);
						$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacaoAcesso,$oUsuario->getId(),utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
					...
					$nIdTipoTransacaoAcesso = $oFachadaSeguranca->recuperaTipoTransacaoPorDescricaoCategoria("Acesso","Login",$sBanco);
					$sObjetoAcesso = "VERIFICAR  ERRO 4: Tentativa de Login falhou. USUÁRIO EXISTE MAS HOUVE ERRO NA SENHA. Login do usuário: ".utf8_decode($_POST['fLogin'])." Senha: ".$_POST['fSenha'];
					$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacaoAcesso("",$nIdTipoTransacaoAcesso,$oUsuario->getId(),utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
					$oFachadaSeguranca->insereTransacaoAcesso($oTransacaoAcesso,$sBanco);
				}//if ($oUsuario->getSenha() == $sSenha){
			}else{
				$nIdTipoTransacaoAcesso = $oFachadaSeguranca->recuperaTipoTransacaoPorDescricaoCategoria("Acesso","Login",$sBanco);
				$sObjetoAcesso = "VERIFICAR ERRO 5: Tentativa de Login falhou. USUÁRIO NÃO ENCONTRADO. Login do usuário: ".utf8_decode($_POST['fLogin'])." Senha: ".$_POST['fSenha'];
				$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacaoAcesso("",$nIdTipoTransacaoAcesso,ID_USUARIO_SISTEMA,utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
				$oFachadaSeguranca->insereTransacaoAcesso($oTransacaoAcesso,$sBanco);
			}//if(is_object($oUsuario)){
		}else{
			$nIdTipoTransacaoAcesso = $oFachadaSeguranca->recuperaTipoTransacaoPorDescricaoCategoria("Acesso","Login",$sBanco);
			$sObjetoAcesso = "VERIFICAR ERRO 6: Tentativa de Login falhou. USUÁRIO NÃO ENCONTRADO. Login do usuário: ".utf8_decode($_POST['fLogin'])." Senha: ".$_POST['fSenha'];
			$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacaoAcesso("",$nIdTipoTransacaoAcesso,ID_USUARIO_SISTEMA,utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
			$oFachadaSeguranca->insereTransacaoAcesso($oTransacaoAcesso,$sBanco);
		}//if(count($voUsuario) == 1){
