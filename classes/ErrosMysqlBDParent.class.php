...
var $oConexao;
	...
	*/
	function recupera($nId) {
		...
		if ($oReg) {
			$oErrosMysql = new ErrosMysql($oReg->id,utf8_encode($oConexao->unescapeString($oReg->erro)),$oReg->id_usuario,utf8_encode($oConexao->unescapeString($oReg->ip)),$oReg->dt_erro,$oReg->publicado,$oReg->ativo);
			return $oErrosMysql;
		}
		return false;
	}
	...
	*/
	function insere($oErrosMysql) {
		$oConexao = $this->getConexao();
		$sSql = "insert into seg_erros_mysql (erro,id_usuario,ip,dt_erro,publicado,ativo) 
				 values ('".utf8_decode($oConexao->escapeString($oErrosMysql->getErro()))."','".$oErrosMysql->getIdUsuario()."','".utf8_decode($oConexao->escapeString($oErrosMysql->getIp()))."','".$oErrosMysql->getDtErro()."','".$oErrosMysql->getPublicado()."','".$oErrosMysql->getAtivo()."')";		
		$oConexao->execute($sSql);		
	...
	*/
	function altera($oErrosMysql) {
		...
		$sSql = "update seg_erros_mysql 
				 set    erro = '".utf8_decode($oConexao->escapeString($oErrosMysql->getErro()))."',
						id_usuario = '".$oErrosMysql->getIdUsuario()."',
						ip = '".utf8_decode($oConexao->escapeString($oErrosMysql->getIp()))."',
						dt_erro = '".$oErrosMysql->getDtErro()."',
