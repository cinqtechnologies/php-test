...
class Conexao {
	...
	* @access public
	*/
	function __construct($sServidor = 'BANCO') {
		$this->setServidor($sServidor);
		switch ($sServidor) {
			case 'LOCAL':
				$this->conectaBD("localhost","root","root","cinqtest");
			break;
			case 'BANCO':
	...
	*/
	function conectaBD($sHost,$sUser,$sSenha,$sBanco) {
		//$link = mysqli_init();
		//$port = 3306;
		//$this->pConexao = mysqli_real_connect($link,$sHost,$sUser,$sSenha,$sBanco,$port);
	...
	*/
	function execute($sSql) {
		...
		if (!$this->getConsulta()){
			...
			$sSqlErroExecucao = "insert into seg_erros_mysql (erro,ip) 
				 				values ('".utf8_encode($this->escapeString($this->getErro()))."','".IP_SERVIDOR."')";
			//echo "<br>";print_r($sSqlErroExecucao);echo "<br>";
			$insereErro = mysqli_query($this->getConexao(),$sSqlErroExecucao) or die(mysqli_error($this->getConexao()));
