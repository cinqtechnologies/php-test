	...
	if($nTempoTranscorrido >= 600) {
		...
		if(isset($_SESSION['oLoginAdm']) && $_SESSION['oLoginAdm']->getIdUsuario() != "" && $_SESSION['oLoginAdm']->getIdUsuario() != 0){
			...
			if(is_object($oUsuarioLogoff)){
				...
				$sObjetoLogado = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." passou mais de 10 minutos em inatividade no sistema, liberando nova possibilidade de acesso para o login: ".utf8_decode($oUsuarioLogoff->getLogin());
				$oTransacaoLogado = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$oUsuarioLogoff->getId(),utf8_encode($sObjetoLogado),IP_USUARIO,DATAHORA,1,1);
				$oFachadaSeguranca->alteraUsuario($oUsuarioLogoff,$oTransacaoLogado,BANCO);
				
				//TRANSACAO
				$sObjetoLogoff = "Logout automático efetuado por prazo de mais de 10 minutos de inatividade ocorrido. Usuario: ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario()).", Login do usuário: ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getLogin());
				...
				//TRANSACAO
				$sObjetoLogoff = "VERIFICAR ERRO INATIVIDADE: Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." não encontrado no sistema para logout automático!";
			...
			$oTransacaoOff = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,ID_USUARIO_SISTEMA,utf8_encode($sObjetoLogoff),IP_USUARIO,DATAHORA,1,1);
			$oFachadaSeguranca->insereTransacao($oTransacaoOff,BANCO);
		}//if(isset($_SESSION['oLoginAdm']) && $_SESSION['oLoginAdm']->getIdUsuario() != "" && $_SESSION['oLoginAdm']->getIdUsuario() != 0){
		...
		header("location: ".SITE."painel/index.php?bErro=0");
		exit();
