...
if(!$bPermissao){
	...
	if($sOP != "Excluir"){
		$sObjetoAcesso = "VERIFICAR: Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou ".$sOP." informações na gerência de usuários, porém não possui permissão para isso! Segue abaixo as informações:<br />";
		if($sOP == "Cadastrar"){
			if(isset($_POST['fNome']))
				$sObjetoAcesso .= "Usuário: ".utf8_encode($_POST['fNome'])."<br />";
			if(isset($_POST['fLogin']))
				$sObjetoAcesso .= "Login: ".utf8_encode($_POST['fLogin'])."<br />";
			if(isset($_POST['fIdGrupoUsuario'])){
				...
				$sObjetoAcesso .= "Grupo de Usuário: ".utf8_decode($sGrupoUsuarioAcesso)."<br />";
			}//if(isset($_POST['fIdGrupoUsuario'])){
			...
			if($sOP == "Alterar"){
				...
				if(is_object($oUsuarioAuxiliar)){
					...
					if($oUsuarioAuxiliar->getLogin() != $_POST['fLogin'])
						$sObjetoAcesso .= "Login: ".utf8_decode($oUsuarioAuxiliar->getLogin())." --> ".utf8_encode($_POST['fLogin'])."<br />";
					if($oUsuarioAuxiliar->getIdGrupoUsuario() != $_POST['fIdGrupoUsuario'])
						$sObjetoAcesso .= "Grupo de Usuário: ".utf8_decode($sGrupoUsuarioAuxiliar)." --> ".utf8_decode($sGrupoUsuarioAuxiliarNovo)."<br />";
...
if (isset($sOP) && $sOP != "Excluir"){
	...
	$oUsuario = $oFachadaSeguranca->inicializaUsuario($_POST['fIdUsuario'],$_POST['fIdGrupoUsuario'], utf8_encode($_POST['fNome']),utf8_encode($_POST['fLogin']),$_POST['fSenha'],$_POST['fEmail'],$bLogado,$_POST['fDataCadastro'],$bPublicado,$bAtivo);
	//print_r($oUsuario);
	//die();
...
switch($sOP){
	case "Cadastrar":
		// TRANSACAO
		$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou o cadastro de novo usuário de nome ".$_POST['fNome']." com o login ".$_POST['fLogin'];
		...
		if (!$nId){
			...
			$sObjetoAcesso = "VERIFICAR: Tentativa de cadastro de novo usuário falhou. Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou cadastrar novo usuário, porém houve erro no cadastro. Informações que seriam cadastradas: <br />";
			if($_POST['fNome'] != "")
				$sObjetoAcesso .= "Usuário: ".utf8_encode($_POST['fNome'])."<br />";
			if($_POST['fLogin'] != "")
				$sObjetoAcesso .= "Login: ".utf8_encode($_POST['fLogin'])."<br />";
			if($_POST['fIdGrupoUsuario'] != "")
				$sObjetoAcesso .= "Grupo de Usuário: ".utf8_decode($sGrupoUsuarioAuxiliarNovo)."<br />";
			if($_POST['fSenha'] != "")
				$sObjetoAcesso .= "Senha: ".$_POST['fSenha']."<br />";
			if($_POST['fEmail'] != "")
	...
	case "Alterar":
		...
		if(is_object($oUsuarioAuxiliar)){
			...
			// TRANSACAO
			$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou alteração de Informações do Usuário ".$oUsuarioAuxiliar->getNmUsuario()."<br /> Modificações realizadas: <br />";
			if($oUsuarioAuxiliar->getNmUsuario() != $_POST['fNome'])
				$sObjeto .= "Usuário: ".utf8_decode($oUsuarioAuxiliar->getNmUsuario())." --> ".utf8_encode($_POST['fNome'])."<br />";
			if($oUsuarioAuxiliar->getLogin() != $_POST['fLogin'])
				$sObjeto .= "Login: ".utf8_decode($oUsuarioAuxiliar->getLogin())." --> ".utf8_encode($_POST['fLogin'])."<br />";
			if($oUsuarioAuxiliar->getIdGrupoUsuario() != $_POST['fIdGrupoUsuario'])
				$sObjeto .= "Grupo de Usuário: ".utf8_decode($sGrupoUsuarioAuxiliar)." --> ".utf8_decode($sGrupoUsuarioAuxiliarNovo)."<br />";
			if($oUsuarioAuxiliar->getSenha() != $_POST['fSenha'] && $_POST['fSenha'] != "")
	...
	case "Excluir":
		...
		foreach($_POST['fIdUsuario'] as $nIdUsuario) {
			if ($oFachadaSeguranca->presenteUsuario($nIdUsuario,BANCO)){
				...
				$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou a desativação do Usuário ".utf8_decode($oUsuarioAuxiliar->getNmUsuario())." com o login ".utf8_decode($oUsuarioAuxiliar->getLogin());
				$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
