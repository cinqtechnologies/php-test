...
if(!$bPermissao){
	...
	if($sOP != "Excluir"){
		$sObjetoAcesso = "VERIFICAR: Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou ".utf8_decode($sOP)." informações na gerência de grupos de usuários, porém não possui permissão para isso! Segue abaixo as informações:<br />";
		if($sOP == "Cadastrar"){
			if(isset($_POST['fNome']))
				$sObjetoAcesso .= "Grupo de Usuários: ".utf8_encode($_POST['fNome'])."<br />";
			...
			if($sOP == "Alterar"){
				...
				if(is_object($oGrupoUsuarioAuxiliar)){
					...
					if($oGrupoUsuarioAuxiliar->getNmGrupoUsuario() != $_POST['fNome'])
						$sObjetoAcesso .= "Grupo de Usuários: ".utf8_decode($oGrupoUsuarioAuxiliar->getNmGrupoUsuario())." --> ".utf8_encode($_POST['fNome'])."<br />";
			...
			foreach($_POST['fIdGrupoUsuario'] as $nIdGrupoUsuario) {
				if ($oFachadaSeguranca->presenteGrupoUsuario($nIdGrupoUsuario,BANCO)){
					...
					if(is_object($oGrupoUsuarioAuxiliarAcesso)){
						$sObjetoAcesso .= "Tentou excluir o grupo de usuários ".utf8_decode($oGrupoUsuarioAuxiliarAcesso->getNmGrupoUsuario())." de id=".$oGrupoUsuarioAuxiliarAcesso->getId()."<br />";
					}//if(is_object($oGrupoUsuarioAuxiliarAcesso)){
				}//if ($oFachadaSeguranca->presenteGrupoUsuario($nIdGrupoUsuario,BANCO)){
			}//foreach($_POST['fIdGrupoUsuario'] as $nIdGrupoUsuario) {
		}else{
...
if (isset($sOP) && $sOP != "Excluir"){
	...
	$oGrupoUsuario = $oFachadaSeguranca->inicializaGrupoUsuario($_POST['fIdGrupoUsuario'], utf8_encode($_POST['fNome']),$_POST['fDataCadastro'],$bPublicado,$bAtivo);
...
switch($sOP){
	case "Cadastrar":
		// TRANSACAO
		$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou o cadastro de novo grupo de usuários de nome ".utf8_encode($_POST['fNome']);
		...
		if (!$nIdGrupoUsuario){
			$sObjetoAcesso = "VERIFICAR: Tentativa de cadastro de novo grupo de usuários falhou. Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou cadastrar novo grupo de usuários, porém houve erro no cadastro. Informações que seriam cadastradas: <br />";
			if($_POST['fNome'] != "")
				$sObjetoAcesso .= "Grupo de Usuário: ".utf8_encode($_POST['fNome'])."<br />";
			...
			//TRANSACAO
			$sObjeto = "Habilitando Permissão de Login para o Grupo de Usuários ".utf8_encode($_POST['fNome'])." de id: ".$nIdGrupoUsuario;
			$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
			...
			//TRANSACAO
			$sObjeto = "Habilitando Permissão de Logout para o Grupo de Usuários ".utf8_encode($_POST['fNome'])." de id: ".$nIdGrupoUsuario;
			$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
			...
			//TRANSACAO
			$sObjeto = "Habilitando Permissão de Alteração de Senha para o Grupo de Usuários ".utf8_encode($_POST['fNome'])." de id: ".$nIdGrupoUsuario;
			$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
	...
	case "Alterar":
		...
		if(is_object($oGrupoUsuarioAuxiliar)){
			// TRANSACAO
			$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou alteração de Informações do Grupo de Usuários ".utf8_decode($oGrupoUsuarioAuxiliar->getNmGrupoUsuario())."<br /> Modificações realizadas: <br />";
			if($oGrupoUsuarioAuxiliar->getNmGrupoUsuario() != $_POST['fNome'])
				$sObjeto .= "Grupo de Usuários: ".utf8_decode($oGrupoUsuarioAuxiliar->getNmGrupoUsuario())." --> ".utf8_encode($_POST['fNome'])."<br />";
			if($oGrupoUsuarioAuxiliar->getPublicado() != $bPublicado){
				if($oGrupoUsuarioAuxiliar->getPublicado() == 1)
					$sPublicadoAtual = "Sim";
				else
	...
	case "Excluir":		
		...
		foreach($_POST['fIdGrupoUsuario'] as $nIdGrupoUsuario) {
			...
			if(is_object($oGrupoUsuarioAuxiliar)){
				// TRANSACAO
				$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." efetuou a desativação do Grupo de Usuários ".utf8_decode($oGrupoUsuarioAuxiliar->getNmGrupoUsuario()).", o que ocasiona a desativação de suas respectivas permissões!";
				$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacao,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);

				//$bResultado &= $oFachadaSeguranca->desativaGrupoUsuario($nIdGrupoUsuario,$oTransacao,BANCO);
                                $bResultado &= $oFachadaSeguranca->excluiGrupoUsuario($nIdGrupoUsuario,$oTransacao,BANCO);
				if($bResultado == true)
