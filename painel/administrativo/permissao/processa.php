...
if(!$bPermissao){
	// TRANSACAO
	$sObjetoAcesso = "VERIFICAR: Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou alterar as permissões na gerência de grupos de usuários do grupo ".utf8_decode($sGrupoUsuarioAuxiliar).", porém não possui permissão para isso!";
...
if(isset($_POST['fIdTipoTransacao']) && count($_POST['fIdTipoTransacao']) > 0){
	...
	$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." iniciou alteração de permissões para o Grupo de Usuários ".utf8_decode($sGrupoUsuarioAuxiliar)." o que ocasiona uma remoção inicial de todas as permissões deste grupo!";
	...
	foreach($vIdTipoTransacao as $nIdTipoTransacao){
		...
		if(is_object($oTipoTransacaoAuxiliar)){
			...
			$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." habilitou a permissão de ".utf8_decode($sTipoTransacaoAuxiliar)." na seção ".utf8_decode($sCategoriaTipoTransacaoAuxiliar)." para o Grupo de Usuários ".utf8_decode($sGrupoUsuarioAuxiliar);
			$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacaoPrincipal,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
		}
	...
	$oFachadaSeguranca->desativaPermissaoPorGrupoUsuario($_POST['fIdGrupoUsuario'],BANCO);
	$sObjeto = "Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." iniciou alteração de permissões para o Grupo de Usuários ".utf8_decode($sGrupoUsuarioAuxiliar).", entretanto não foram encontradas novas permissões para serem adicionadas, por isso, todas as permissões deste grupo estão desativadas!";
	$oTransacao = $oFachadaSeguranca->inicializaTransacao("",$nIdTipoTransacaoPrincipal,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjeto),IP_USUARIO,DATAHORA,1,1);
	$oFachadaSeguranca->insereTransacao($oTransacao,BANCO);
}

if (!$bResultado){
	$sObjetoAcesso = "VERIFICAR: Usuário ".utf8_decode($_SESSION['oLoginAdm']->oUsuario->getNmUsuario())." tentou alterar as permissões para o Grupo de Usuários ".utf8_decode($sGrupoUsuarioAuxiliar).", porém ocorreu um erro na alteração!";
	$oTransacaoAcesso = $oFachadaSeguranca->inicializaTransacaoAcesso("",$nIdTipoTransacaoPrincipal,$_SESSION['oLoginAdm']->getIdUsuario(),utf8_encode($sObjetoAcesso),IP_USUARIO,DATAHORA,1,1);
	$oFachadaSeguranca->insereTransacaoAcesso($oTransacaoAcesso,BANCO);
