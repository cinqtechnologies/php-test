...
<form action="{{route('prod.persistEdit',['id' => $produto->id])}}" method="post">
	<div class="form-group">
			...
			<div class="col-md-6">
				...
				<div class="row">
					<div class="col-md-4">
						<label for="inputImagePath">Image Path</label> <input type="text" value="{{$produto->ImagePath}}"
				...
				<div class="col-md-4">
					<div class="form-group">
						<label for="selectRetailer">Retailer</label> <select
							...
							<option value="{{$ret->id}}" @if($ret->id == $produto->IdRetailer)selected @endif>{{$ret->Name}}</option>
							@endforeach
						</select>
					</div>
				</div>
