	...
	$('#btnSair').click(function(){
		if(confirm('Deseja realmente sair do sistema?')){
			...
			$.ajax({
				type: "POST",
				//LOCAL
				url: window.location.origin+"/painel/administrativo/login/processa.php",
				//ONLINE
				//url: window.location.origin+"/sistema/painel/administrativo/login/processa.php",
				data: {sOP:"Logoff"},
				success: function(resp){
					if(resp == 1){
						//LOCAL
						window.open(window.location.origin+'/painel/index.php','_self');
						//ONLINE
						//window.open(window.location.origin+'/sistema/painel/index.php','_self');
					}
				}
			});
		}
